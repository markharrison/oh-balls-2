...
export class SceneManager {
    ...
    stopJittering() {
        ...
        bodies.forEach((body) => {
            if (body.label === 'ball' && !body.isStatic) {
                // Use built-in speed property instead of calculating velocity magnitude manually
                if (body.speed < 0.1) {
                    // Increased threshold to be more aggressive
                    // Stop linear motion
                    Matter.Body.setVelocity(body, { x: 0, y: 0 });

                    // Clear accumulated forces that could cause micro-movements
                    body.force.x = 0;
                    body.force.y = 0;
                }
                ...
                // Stop angular motion and torque (use angularSpeed for consistency)
